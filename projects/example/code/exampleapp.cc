...
namespace Example
{
	...
	*/
	...
	void ExampleApp::pointLocation(Node* tree, glm::vec2 point)
	{
		...
		//	// kommentera ut "=" i else satsen för att få bort strecken
		//	if (onSegment(c, point, c_i)) {
		//		std::cout << "point on left line" << std::endl;
		//		ExampleApp::pointOnLine(tree, point);
		//	}

		//	if (onSegment(c_m, point, c_i)) {
		//		std::cout << "point on middle line" << std::endl;
		//		ExampleApp::pointOnLine(tree, point);
		//	}

		//	if (onSegment(c, point, c_m)) {
		//		std::cout << "point on right line" << std::endl;
		//		ExampleApp::pointOnLine(tree, point);
		//	}
		...
		window->SetKeyPressFunction([this](int32 key, int32 scancode, int32 action, int32 mods)
			{
				if (key == 256 && action == GLFW_PRESS) {
					...
					//glm::vec2 test = glm::vec2(0.577 / 2.0f, 0.333 / 2.0f);
